...
def main(args):
    ...
    if args.eval:
        ...
        vals = args.test_dataset #os.listdir(args.eval_data_path)
        args.eval_data_path = "/path/to/dummy/testset"
        # if len(vals) == 16:
        ...
        for v_id, val in enumerate(vals):
            ...
            dataset_val = TestDataset(is_train=False, args=args, eval_dataset_name = val)
            ...
            test_stats, acc, ap, auc, eer = evaluate(data_loader_val, model, device)
            print(f"Accuracy of the network on {len(dataset_val)} test images: {test_stats['acc1']:.5f}%")
        
            print(f"test dataset is {val} acc: {acc}, ap: {ap}, auc: {auc}, eer: {eer}")
            print("***********************************")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            test_stats, acc, ap, auc, eer = evaluate(data_loader_val, model, device, use_amp=args.use_amp)
            ...
            if args.model_ema and args.model_ema_eval:
                test_stats_ema, acc, ap, auc, eer = evaluate(data_loader_val, model_ema.ema, device, use_amp=args.use_amp)
