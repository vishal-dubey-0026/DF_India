...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    def load_rgb(self, file_path):
        ...
        if True: # fake image, for gemini watermark on bottom-right
            crop_h, crop_w = img.shape[0], int(img.shape[1] * 0.9)
            img = img[0: crop_h, 0: crop_w, :].copy()
            # img_w, img_h = image.size
            # crop_h, crop_w = img_h, int(img_w * 0.9)
            # image = image.crop((0, 0, crop_w, crop_h)) #(left, upper, right, lower)
            #print(f"{file_path}, {crop_w}:{crop_h}, {img.shape}")
